...
outDir <- "C:/Claire/GPEM_Postdoc/2_SEQ_Koala_Collaboration/3_game theory/outputs/Case study/TEST_scen3/"
...
#Set up a 2D matrix where each row represents r for each investment value
	...
	outPath <-paste0(outDir, "GrowthRate_r_vsInvestment_by_council_to10m.csv")
...
for(i in 1:n){
	...
	# counter=0
	# oldSpend <- c(NA,NA,NA)
	# while (!identical(currSpend,oldSpend)){
		# counter=counter+1
		# print(currSpend)
		...
		print(currSpend)
		print(oldSpend)
		print(!identical(currSpend,oldSpend))
		if(counter==251){
		break
		}
...
outPath <- paste0(outDir, "PrintVals_test1.csv")
