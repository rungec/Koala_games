...
#coefficient of asymmetry 0=symmetric, 1=highly asymmetric
#assymcoef <- seq(-1,1,0.1)
nreps <- 100
 #the mean migration rate
 
#generate data popn cor of 0.3 but actual corr of dataset may not be 0.3
#How to make the values positive?
gendata <- function(assymcoef) {
			corrmat <- matrix(data=rep(assymcoef,  6*6), ncol=6, nrow=6)#correlation matrix
			diag(corrmat) <- 1
			meanlog <- rep(2,6)
		return(rlnorm.rplus(n=nreps,meanlog=meanlog,varlog=corrmat))
}

d<-mvrnorm(n = nreps, mu=meanlog, Sigma=corrmat, tol = 1e-6, empirical = TRUE)#generate data with actual cor of 0.3 (but not lognormal)
...
#loop ThreePatchMod over each row of this matrix to get the benefit arrays
	...
	BenefitS1[,c("brisD", "logaD", "redlD")] <- 1-BenefitS1[,c("bris", "loga", "redl")]/Popn2008
