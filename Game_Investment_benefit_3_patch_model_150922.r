...
#loop ThreePatchMod over each row of rMatrix to get the benefit arrays
...
for(i in 1:n){
	...
	currSpend <- c(newChoices[1], newChoices[2], newChoices[3])
	}
}
...
#ROI for scenario 3
	S3benDF <- c()
	for (i in 1:nrow(EquilibriumSpendOptions)) {
			budget <- as.numeric(EquilibriumSpendOptions[i,])
			newr <- Map(ReturnonInvest, budget, 1:3) # the new growth rate due to investment
			Pars <- as.list(c(r1=newr[1], r2=newr[2], r3=newr[3], K1, K2, K3, Z))
			NewN <- ode(Yini,Times,ThreePatchModC,Pars)
			numKoala <- NewN[nrow(NewN),2:4]
			...
			numKoalattl <- sum(numKoala)
			declKoalattl <- 1-numKoalattl/sum(Popn2008)
			newRow <- append(budget, c(numKoala, numKoalattl, declKoala, declKoalattl))
			S3benDF <- rbind(S3benDF, newRow)			
	}

	S3benDF <- data.frame(S3benDF)
	names(S3benDF) <- c("bris_invest", "loga_invest", "redl_invest", "numKoala_bris", "numKoala_loga", "numKoala_redl", "numKoala_overall", "decline_bris", "decline_loga", "decline_redl", "decline_overall")
outPath <- paste0(outDir, "ROI_local50decline_casestudy.csv")
write.csv(S3benDF, outPath)